        ...
        while True:
            ...
            try:
                ...
                for idx, item in enumerate(board_entries):
                    ...
                    if item["qid"] is None:
                        logger.warning(
                            f"Unparsed URL @ {item['url']} ranking {idx} in crawl {crawl_id}.")
                    else:
                        try:
                            ...
                            if len(e.args) > 0 and isinstance(e.args[0], requests.Response):
                                logger.exception(
                                    f"{e}; {e.args[0].status_code}; {e.args[0].text}")
                            else:
                                logger.exception(f"{str(e)}")
                        else:
                            logger.info(
                                f"Get question detail for {item['title']}: raw detail length {len(detail['raw']) if detail['raw'] else 0}")
                    try:
                ...
                logger.exception(
                    f"Crawl {crawl_id} encountered an exception {e}. This crawl stopped.")
            self.sleep("interval_between_board",
                       delta=(begin_time - time.time()))
...
"""
    ...
    def get_board(self) -> list:
        ...
        hots = soup.find_all("div", class_="HotItem-content")
        if(len(hots) == 0):
            raise

        for hot_item in hots:
            a_hot = {}
            url_title_excerpt = hot_item.find_all('a')
            if(len(url_title_excerpt)):
                a_hot['url'] = url_title_excerpt[0].attrs['href']
                a_hot['title'] = url_title_excerpt[0].attrs['title']
                a_hot['excerpt'] = ""
                temp = url_title_excerpt[0].find_all(class_="HotItem-excerpt")
                if(len(temp)):
                    a_hot['excerpt'] = temp[0].string
                a_hot['qid'] = a_hot['url'].split("/")[-1]

                try:
                    h = [_ for _ in hot_item.find_all(
                        "div", class_="HotItem-metrics HotItem-metrics--bottom")[-1].children][2]
                except:
                    h = [_ for _ in hot_item.find_all(
                        "div", class_="HotItem-metrics")[-1].children][2]
            try:
                a_hot['heat'] = h.split(" ")[0]
            except:
                a_hot['heat'] = ''
    ...
    def get_question(self, qid: int) -> dict:
        ...
        """
        ans = {"created": "", "followerCount": "", "visitCount": "",
               "answerCount": "", "title": "", "raw": "", "hit_at": ""}
        ...
        resp = requests.get(url, headers=self.settings['headers'])
        soup = BS(resp.text, 'lxml')
        question = soup.find_all("div", class_='QuestionPage')

        if(len(question) != 1):
            raise

        # I do not know how to deal with the timezone,but this should be correct as long as the strptime function deal the
        # date_created and time_864000 in the same timezone
        try:
            time_864000 = "1970-01-11T00:00:00.000Z"
            time_stamp_864000 = int(datetime.strptime(
                time_864000, "%Y-%m-%dT%H:%M:%S.000Z").timestamp())
            date_created = question[0].find_all("meta", itemprop="dateCreated")[
                0].attrs['content']
            ans['created'] = int(datetime.strptime(
                date_created, "%Y-%m-%dT%H:%M:%S.000Z").timestamp()) + 864000 - time_stamp_864000
        except:
            pass

        try:
            numbers = question[0].find_all(
                "strong", class_="NumberBoard-itemValue")
            ans['followerCount'] = numbers[0].attrs['title']
            ans['visitCount'] = numbers[-1].attrs['title']
        except:
            pass

        try:
            question_main = soup.find_all("div", class_="Question-main")
            number = question_main[0].find_all("h4", class_="List-headerText")
            ans['answerCount'] = (number[0].find("span").text).split(" ")[0]
        except:
            pass

        try:
            question_title = question[0].find(
                "h1", class_="QuestionHeader-title").text
            ans['title'] = question_title
        except:
            pass

        try:

            ans['raw'] = question[0].find("span", itemprop="text").text
        except:
            pass

        ans["hit_at"] = time.time()
